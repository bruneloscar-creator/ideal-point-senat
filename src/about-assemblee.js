import { loadScatterDeputies, mountIdealScatter } from './idealScatter.js';
...
document.documentElement.dataset.chamber = 'assemblee';
...
async function init() {
  if (!canvas) return;
  try {
    const { senators, groupColors } = await loadScatterDeputies('/assets/deputies/manifest.json');
    ...
    console.warn('Carte Ideal Point AN indisponible:', err);
    if (frame) {
      frame.classList.add('is-error');
      frame.setAttribute('data-error', t('aboutAn.map.error'));
    }
  }
}
